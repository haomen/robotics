...
Dis_measure_matrix=(Dis_measure_matrix+Dis_measure_matrix')/2;
Angle_measure_matrix_noise=delta*rands(n,n);
Angle_measure_matrix=map(angle_matrix+Angle_measure_matrix_noise);
Angle_measure_matrix=Angle_measure_matrix.*comm_matrix;
for i=1:n
    for j=1:i
        if (comm_matrix(i,j)~=0)
Angle_measure_matrix(i,j)=(Angle_measure_matrix(i,j)+map(pi+Angle_measure_matrix(j,i)))/2;
Angle_measure_matrix(j,i)=map(pi+Angle_measure_matrix(i,j));
        end
    end
end
...
display(['total cpu time:',num2str(t_time),' seconds'])
...
figure(7),clf(7),hold on, box on
clear i
E_dis_vec=zeros(size(xstore,3),1);
for j=1:size(xstore,3)
xdata=xstore(:,:,j);
dis_m=xdata(:,1)+xdata(:,2)*i;
dis_m=-dis_m*ones(1,n)+ones(n,1)*conj(dis_m');
Dis_m=abs(dis_m);
Ang_m=angle(dis_m);
E_dis(j)=sum(abs((Dis_m-dis_matrix).*comm_matrix))/(sum(comm_matrix));
E_angle(j)=sum(abs((Ang_m-angle_matrix).*comm_matrix))/(sum(comm_matrix));
end
E_angle=rad2deg(E_angle);
subplot(2,1,1)
plot(0:size(xstore,3)-1,E_dis)
title('distance error')
subplot(2,1,2)
plot(0:size(xstore,3)-1,E_angle)
title('angle error')
X_es=xstore(:,:,end);
X_es=X_es(:,1)+i*X_es(:,2);
X_es=X_es-mean(X_es);
X_es=X_es+mean(node_coor);
...
ylim([-0.15,1.15])
box on
scatter(real(X_es),imag(X_es),15,'r');
for i=1:n
    figure(100),hold on,
plot([real(node_coor(i));real(X_es(i))],[imag(node_coor(i));imag(X_es(i))],'g');
end
